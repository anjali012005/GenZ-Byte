...
function SessionPage() {
  ...
  }, [session, user, loadingSession, isHost, isParticipant, id, joinSessionMutation.isPending, refetch]);
  ...
  const handleRunCode = async () => {
    if (selectedLanguage !== "javascript") {
      ...
      setOutput({
        success: false,
        error: `${languageName} execution is coming soon. For now, this live session runner supports JavaScript only.`,
      });
    ...
    if (result.success) {
      const expectedOutput = problemData?.expectedOutput?.javascript;

      if (expectedOutput && checkIfTestsPassed(result.output, expectedOutput)) {
        triggerConfetti();
      }
    } else {
